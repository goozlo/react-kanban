...
export const Accordion = ({ options, width = '100%' }) => {
  ...
  return (
    <div className="accordion">
      <svg
        className="accordion__action-menu"
      ...
      <ul className={`accordion__frame ${isVisible && 'active'}`} style={{ width }}>
        {options.map((option, index) => {
          const { label, color } = option;
          return (
            <li className="accordion__frame__option" style={{ color }} onClick={() => setIsVisible(false)} key={index}>
              {label}
            </li>
          );
        })}
